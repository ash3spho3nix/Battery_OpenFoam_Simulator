...
class InterfaceFactoryEnhanced:
    """
    ...
    def _create_ui_enhanced(
        ...
        ui_path = UILoaderEnhanced.get_ui_path(ui_name, ui_config.get_ui_base_path())
        
        # Validate UI file integrity before loading
        if not UILoaderEnhanced.validate_ui_integrity(ui_path):
            logger.warning(f"UI file integrity check failed: {ui_path}")
            return None
        
        # Load UI with enhanced validation
        return UILoaderEnhanced.load_ui_file(ui_path, parent, validate_ui=True)
    ...
    def _should_try_ui_loading(
        ...
        if ui_config.mode == UILoadingMode.UI_FILES:
            ...
            ui_name = cls._get_ui_name(interface_type)
            if loader_type == 'enhanced':
                from src.gui.ui_loader_enhanced import UILoaderEnhanced
                return UILoaderEnhanced.ui_file_exists(ui_name, ui_config.get_ui_base_path())
            else:
                from src.gui.ui_loader import UILoader
    ...
    def _test_ui_files(cls, interface_type: str, ui_config: Optional['UIConfig']) -> Dict[str, Any]:
        ...
        try:
            ...
            from src.gui.ui_loader_enhanced import UILoaderEnhanced
            ui_name = cls._get_ui_name(interface_type)
            ui_path = UILoaderEnhanced.get_ui_path(ui_name, ui_config.get_ui_base_path() if ui_config else None)
            
            results['enhanced']['file_exists'] = os.path.exists(ui_path)
            results['enhanced']['integrity_valid'] = UILoaderEnhanced.validate_ui_integrity(ui_path) if results['enhanced']['file_exists'] else False
    ...
    def _test_enhanced_ui(cls, interface_type: str, ui_config: Optional['UIConfig']) -> Dict[str, Any]:
        ...
        try:
            ...
            if UILoaderEnhanced.validate_ui_integrity(ui_path):
                # Try to load without parent (for testing)
                widget = UILoaderEnhanced.load_ui_file(ui_path, parent=None, validate_ui=True)
